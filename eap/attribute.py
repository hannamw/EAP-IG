...
def make_hooks_and_matrices(model: HookedTransformer, graph: Graph, batch_size:int , n_pos:int):
    ...
    def activation_hook(t: torch.Tensor, index, add=True):
        def hook_fn(activations, hook):
            # activations is (batch, pos, ???, hidden)
...
def get_activations(model: HookedTransformer, graph: Graph, clean: List[str], corrupted: List[str], metric: Callable[[Tensor], Tensor], labels):
...
def get_activations_ig(model: HookedTransformer, graph: Graph, clean: List[str], corrupted: List[str], metric: Callable[[Tensor], Tensor], labels, steps=30):
    batch_size = len(clean)
    clean_tokens = model.to_tokens(clean, prepend_bos=True, padding_side='right')
    corrupted_tokens = model.to_tokens(corrupted, prepend_bos=True, padding_side='right')
    attention_mask = get_attention_mask(model.tokenizer, clean_tokens, True)
