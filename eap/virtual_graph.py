...
class Node:
    ...
    def __init__(self, name: str, layer:int, in_hook: List[str], out_hook: str, index: Tuple, 
                 graph:'VirtualGraph', qkv_inputs: Optional[List[str]]=None):
        self.name = name
        self.layer = layer
...
class LogitNode(Node):
    def __init__(self, n_layers:int, graph: 'VirtualGraph'):
        name = 'logits' 
        index = slice(None) 
...
class MLPNode(Node):
    def __init__(self, layer: int, graph: 'VirtualGraph'):
        name = f'm{layer}'
        index = slice(None)
...
class AttentionNode(Node):
    head: int
    def __init__(self, layer:int, head:int, graph: 'VirtualGraph'):
...
class InputNode(Node):
    def __init__(self, graph: 'VirtualGraph'):
        name = 'input' 
        index = slice(None) 
...
#     hook: str
#     index: Tuple
#     def __init__(self, graph: 'VirtualGraph', parent: str, child: str, qkv:Optional[Literal["q", "k", "v"]]=None):
#         self.graph = graph
...
class VirtualGraph:
    ...
    def str_to_node(self, s: str):
        if not s:
            raise ValueError('Empty string input')
        if s == 'input':
            return InputNode(self)
        elif s == 'logits':
            return LogitNode(self.n_layers, self)
        elif s[0] == 'm':
            layer = int(s[1:])
            assert 0 <= layer < self.n_layers, f"Invalid layer number: {layer}"
            return MLPNode(layer, self)
        elif s[0] == 'a':
            layer, head = s.split('.')
            layer = int(layer[1:])
            head = int(head[1:])
            assert 0 <= head < self.n_heads, f"Invalid head number: {head}"
            assert 0 <= layer < self.n_layers, f"Invalid layer number: {layer}"
            return AttentionNode(layer, head, self)
        else:
            raise ValueError(f"Invalid node: {s}")
        
    def prev_index(self, node: Node):
        return VirtualGraph._prev_index(self.cfg, node.name)

    @classmethod
    def _prev_index(cls, cfg, node_name: str) -> Union[int, slice]:
        ...
        if node_name == 'input':
            ...
            return cls._n_forward(cfg)
        elif node_name[0] == 'm':
            layer = int(node_name[1:])
            if cfg['parallel_attn_mlp']:
                return 1 + layer * (cfg['n_heads'] + 1)
            else:
    ...
    def forward_index(self, node:str, attn_slice=True) -> int:
        return VirtualGraph._forward_index(self.cfg, node.name, attn_slice)        
    ...
    def backward_index(self, node:str, qkv=None, attn_slice=True) -> int:
        return VirtualGraph._backward_index(self.cfg, node.name, qkv, attn_slice)
    ...
    @classmethod
    def from_model(cls, model_or_config: Union[HookedTransformer,HookedTransformerConfig, Dict], neuron_level: bool = False, node_scores: bool = False) -> 'VirtualGraph':
        ...
        for layer in range(graph.cfg['n_layers']):
            ...
            for attn_node in attn_nodes:
                for letter in 'qkv':
                    ...
                    graph.real_edge_mask[:graph.prev_index(attn_node), graph.backward_index(attn_node, qkv=letter)] = True
            ...
            graph.real_edge_mask[:graph.prev_index(mlp_node), graph.backward_index(mlp_node)] = True


        logit_node = LogitNode(graph.cfg['n_layers'], graph)     
        graph.nodes['logits'] = logit_node
        graph.real_edge_mask[:graph.prev_index(logit_node), graph.backward_index(logit_node)] = True
    ...
    @classmethod
    def from_json(cls, json_path: str) -> 'VirtualGraph':
        """
        ...
        g = VirtualGraph.from_model(d['cfg'], neuron_level=True, node_scores=True)
    ...
    @classmethod
    def from_pt(cls, pt_path: str) -> 'VirtualGraph':
        """
        ...
        g = VirtualGraph.from_model(d['cfg'])
